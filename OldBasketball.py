...
player1_compare = 'Paolo'
player2_compare = 'Jackson'

# Call the generalized function
player_pair_dynamics = calculate_player_pair_team_dynamics_win_percentage(games_v3, player1_compare, player2_compare)

# Print spacing
print(f"\n\n{player1_compare}'s Win Percentage based on {player2_compare}'s Team Presence:")
print("-" * 75) # Adjust separator length

# Define column widths for the new table dynamically
description_col_width = max(len(f"{player1_compare} & {player2_compare} (Same Team)"),
                            len(f"{player1_compare} vs. {player2_compare} (Opposing Teams)")) + 2
win_pct_col_width = len("Win %") + 2
games_col_width = len("Games") + 2

player_pair_header = (f"{'Scenario':<{description_col_width}}"
                     f"{'Win %':<{win_pct_col_width}}"
                     f"{'Games':<{games_col_width}}")
print(player_pair_header)
print("-" * len(player_pair_header))

# Display stats when player1 and player2 are on the same team
same_team_pct = f"{player_pair_dynamics['same_team']['win_percentage'] * 100:.0f}%"
same_team_games = player_pair_dynamics['same_team']['games']

# FIXED: Concatenate the strings directly with '+' or create a single f-string
row_same_team = (f"{player1_compare} & {player2_compare} (Same Team):").ljust(description_col_width) + \
                f"{same_team_pct:<{win_pct_col_width}}" + \
                f"{same_team_games:<{games_col_width}}"
print(row_same_team)

# Display stats when player1 and player2 are on opposing teams
opposing_team_pct = f"{player_pair_dynamics['opposing_team']['win_percentage'] * 100:.0f}%"
opposing_team_games = player_pair_dynamics['opposing_team']['games']

# FIXED: Concatenate the strings directly with '+' or create a single f-string
row_opposing_team = (f"{player1_compare} vs. {player2_compare} (Opposing Teams):").ljust(description_col_width) + \
                    f"{opposing_team_pct:<{win_pct_col_width}}" + \
                    f"{opposing_team_games:<{games_col_width}}"
print(row_opposing_team)